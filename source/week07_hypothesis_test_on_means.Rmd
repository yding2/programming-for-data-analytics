...
For a numerical characteristic of a population, we are often interested in the **population mean**. If the population variance is known, we can use the *standard normal distribution* for inference about the mean. In practice, the population variance is usually unknown and must be estimated from the sample. In that case, the standardized sample mean follows a **t-distribution** when the population is normally distributed.
...
For inference about the difference between two population means, the population variances are generally unknown. We need to consider whether the population variances are equal. The good thing is that the $t$-statistic has the same form in either case; the difference lies in the degrees of freedom.
...
If we want to check if the stem weight with nitrogen is higher, we should conduct the following hypothesis test.
...
noNitro<- c(0.32,0.53 ,0.28, 0.37, 0.47, 0.43, 0.36, 0.42,0.38,0.43)
Nitro<- c(0.26,0.43,0.47,0.49,0.52,0.75,0.79,0.86, 0.62,0.46)
t.test(Nitro,noNitro, var.equal = FALSE, conf.level = .95,alternative = 'greater')
```

Here, p-value is less than $\alpha = 0.05$, we should reject the null hypothesis and the result supports the stem weight with nitrogen is higher.
